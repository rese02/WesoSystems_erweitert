      ...
      <div className="grid grid-cols-1 gap-8 md:grid-cols-3">
        ...
        <div className="space-y-8 md:col-span-2">
            ...
            <Card>
             ...
             <Card>
                <CardHeader><CardTitle>Mitreisende & Kleinkinder</CardTitle></CardHeader>
                <CardContent>
                     {guest && (guest.fellowTravelers?.length > 0 || guest.infants?.length > 0) ? (
                        <ul className="space-y-6">
                        {guest.fellowTravelers?.map((t, i) => (
                             <li key={`ft-${i}`} className="rounded-md border p-4">
                        ...
                        {guest.infants?.map((infant, i) => (
                             <li key={`infant-${i}`} className="rounded-md border p-4 bg-muted/30">
                                <div className='flex items-center gap-3'>
                                    <Baby className="h-5 w-5 text-muted-foreground"/>
                                    <span className='font-medium'>{infant.name}</span>
                                    <Badge variant="secondary">Kleinkind</Badge>
                                </div>
                            </li>
                        ))}
                        </ul>
                    ) : (
                        <p className="text-muted-foreground">Keine Mitreisenden oder Kleinkinder angegeben.</p>
                    )}
                </CardContent>
            </Card>
